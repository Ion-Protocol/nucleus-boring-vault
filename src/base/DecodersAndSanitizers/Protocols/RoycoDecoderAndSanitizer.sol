...
abstract contract RoycoDecoderAndSanitizer is BaseDecoderAndSanitizer {
    ...
    function fillIPOffers(bytes32[] calldata ipOfferHashes,uint256[] calldata fillAmounts,address fundingVault,address frontendFeeRecipient) external pure returns (bytes memory addressesFound) {
    ...
    function claim(address weirollWallet,address incentiveToken,address to) external view returns (bytes memory addressesFound) {
        if (IWeirollWallet(weirollWallet).owner() != boringVault) revert RoycoDecoderAndSanitizer__OwnerMustBeBoringVault();
