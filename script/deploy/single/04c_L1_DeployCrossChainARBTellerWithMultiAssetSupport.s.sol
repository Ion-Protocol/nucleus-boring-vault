...
import {CrossChainARBTellerWithMultiAssetSupportL1} from "src/base/Roles/CrossChain/CrossChainARBTellerWithMultiAssetSupport.sol";
...
contract DeployCrossChainARBTellerWithMultiAssetSupport is BaseScript {
    ...
    function deploy(ConfigReader.Config memory config) public broadcast override returns(address){
        ...
        // Create Contract
        bytes memory creationCode = type(CrossChainARBTellerWithMultiAssetSupportL1).creationCode;
        CrossChainARBTellerWithMultiAssetSupportL1 teller = CrossChainARBTellerWithMultiAssetSupportL1(
            CREATEX.deployCreate3(
                config.tellerSalt,
                abi.encodePacked(creationCode, abi.encode(broadcaster, config.boringVault, config.accountant, config.arbInbox))
            )
        );
        ...
        require(address(teller.inbox()) == config.arbInbox, "OP Teller must have messenger set");
